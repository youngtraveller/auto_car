...
# 单侧最小白像素面积
# 160×120 下 ROI 底部约 60×64px/侧，白线宽 3px → 3×60=180，80 绰绰有余
MIN_SIDE_AREA = 80
...
PRINT_EVERY  = 20           # 每 N 帧打印一次, 打印本身很慢
...
# ==========================================================================
#  十一、视觉识别函数 — 左右分半双线检测（抗墙壁误识别）
# ==========================================================================
def detect_white_center(frame):
    """
    左右分半双线检测（~2ms @ 树莓派）：
      ROI → BGR→HSV → inRange → 左半moments + 右半moments
      关键：只有路面底部的 ROI，墙壁显示在画面上半部 → 被 ROI 直接排除
      左右分半：单侧白色物体（如障碍物）只影响半边 → 不会拉偏中心
    """
    h, w = frame.shape[:2]
    y1 = int(h * ROI_TOP)
    y2 = int(h * ROI_BOTTOM)
    ...
    ML = cv2.moments(mask_left)
    MR = cv2.moments(mask_right)

    has_left  = ML['m00'] > MIN_SIDE_AREA
    has_right = MR['m00'] > MIN_SIDE_AREA

    if has_left and has_right:
        # ✅ 双线：各取质心，取中点
        cxL = int(ML['m10'] / ML['m00'])
        cxR = int(MR['m10'] / MR['m00']) + mid   # 右半边偏移
        raw_cx = (cxL + cxR) // 2
    elif has_left:
        # 只有左线（弯道内侧 / 右线被遮挡）
        raw_cx = int(ML['m10'] / ML['m00'])
    elif has_right:
        # 只有右线（弯道内侧 / 左线被遮挡）
        raw_cx = int(MR['m10'] / MR['m00']) + mid
    else:
        return -1
...
try:
    while True:
        # --- 帧读取（直接 cap.read()，MJPEG+BUFFERSIZE=1 已确保零堆积） ---
        ok, frame = cap.read()
        if not ok:
            continue
